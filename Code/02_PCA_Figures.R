...
# Paths and colors
gam_data_path <- here("Data", "LA_Data",  "NatalToMarine_Processed_GAM.csv")
raw_data_path <- here("Data", "LA_Data",  "NatalToMarine_Processed_RAW.csv")
output_dir <- here("Figures", "PCA Figures")
...
create_additional_pca_plots <- function(gam_data, dataset_name, output_directory) {
  ...
  combined_filepath <- here(output_directory, combined_filename)
...
total_pca_filepath <- here(output_dir, total_pca_filename)
...
overlap_pca_filepath <- here(output_dir, overlap_pca_filename)
...
if(nrow(available_specific_fish) >= 1) {
  ...
  specific_combined_filepath <- here(output_dir, specific_combined_filename)
